package com.memo.memo.topiccontent.repository
...
interface TopicContentRepository : JpaRepository<TopicContent, Long> {
    fun countByMemberAndTopic(
        memberId: Member,
        topic: Topic,
    ): Int

    fun findByMemberIdAndTopicId(
        memberId: Long,
        topicId: Long?,
        pageable: Pageable,
    ): Page<TopicContent>
    ...
    @Query(
        "Select p from TopicContent p where p.topic.id = :topicId AND " +
